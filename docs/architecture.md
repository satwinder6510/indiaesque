...
Auto-generated by `@astrojs/sitemap` integration on every build.
...
**IMPORTANT: Cloudflare Routing**

The sitemap files must be excluded from Worker routing in `astro.config.mjs` to be served as static XML files. Without this, Cloudflare's Worker intercepts the requests and returns 400 errors.

The `routes.extend.exclude` patterns are added to `_routes.json` during build, telling Cloudflare to serve these as static assets.

### Auto-Update Behavior

The sitemap regenerates automatically when:
1. New pages are added (markdown files, new routes)
2. Site is deployed to Cloudflare (build runs)
3. Admin saves content that triggers a rebuild

### Sitemap Rules

- Submit sitemap to Google Search Console: `https://indiaesque.in/sitemap-index.xml`
- Re-submit after adding new cities or major content batches
- Sitemap currently includes all city pages, PAA pages, journal pages, staycations, and static pages
